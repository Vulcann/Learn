...
class CircuitSuite extends CircuitSimulator with FunSuite {
  ...
  test("demux example") {
    val in = new Wire
    val nc = 2
    // val c = (0 until nc).toList.map(_ => new Wire)
    val c = List.fill(nc)(new Wire)
    // val no = pow(2, nc).toInt
    val no = 1 << nc
    // val out = (0 until pow(2, nc).toInt).toList.map(_ => new Wire)
    val out = List.fill(no)(new Wire)
